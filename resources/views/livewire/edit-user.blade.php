<div>
    <title>Volt Laravel Dashboard - Edit User</title>
    <div>
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
            <div class="d-block mb-4 mb-md-0">
                ...
                <p class="mb-0">{{__('Your user editing template.')}}</p>
            </div>
        </div>
        <div class="row">
            <div class="col-12 col-xl-8">
                ...
                <div class="card card-body shadow-sm mb-4">
                    <h2 class="h5 mb-4">{{__('User information')}}</h2>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select wire:model="user.status"
                                    class="form-select mb-0 @error('user.status') is-invalid @enderror" id="status"
                                ...
                                @error('user.status') <div class="invalid-feedback"> {{ $message }} </div> @enderror
                            </div>
                            <div class="col-md-6 mb-3">
                                <label for="role">{{__('Role')}}</label>
                                <select wire:model="user.role_id"
                                    class="form-select mb-0 @error('user.role_id') is-invalid @enderror" id="role"
                                    aria-label="role select example">
                                    @foreach($roles as $role)
                                    <option value="{{ $role->id }}">{{ $role->name }}</option>
                                    @endforeach
                                </select>
                                @error('user.role_id') <div class="invalid-feedback"> {{ $message }} </div> @enderror
                            </div>
                        </div>
                        <div class="row">
                    ...
                    <div class="col-12 mb-4">
                        <div class="card shadow border-0 text-center p-0">
                            ...
                            <div class="card-body pb-5">
                                ...
                                <h4 class="h3">
                                    {{ $this->user->first_name ? $this->user->first_name . ' ' . $this->user->last_name : 'User Name' }}
                                </h4>
                                <h5 class="fw-normal">{{__('Senior Software Engineer')}}</h5>
                                <p class="text-gray mb-4">{{ $this->user->city ? $this->user->city : 'City' }},
                                    {{ $this->user->country ? $this->user->country : 'Country' }}</p>
                                ...
                                <div class="me-3">
                                    ...
                                    <div class="user-avatar xl-avatar">
                                        ...
                                        <img class="rounded avatar-xl" src="{{ $upload->temporaryUrl() }}"
                                            alt="Profile Photo">
                                        @else
                                        <img class="rounded avatar-xl" src="{{ $this->user->avatarUrl() }}"
                                            alt="Profile Photo">
                                        @endif
                                    </div>
                                </div>
