    ...
    <div>
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
            <div class="d-block mb-4 mb-md-0">
                ...
                <h2 class="h4">{{__('Category Management')}}</h2>
                <p class="mb-0">{{__('Your category management dashboard template.')}}</p>
            </div>
            ...
            <div class="btn-toolbar mb-2 mb-md-0">
                <a href="{{ route('new-category') }}" class="btn btn-sm btn-gray-800 d-inline-flex align-items-center"><svg
                    ...
                    </svg> {{__('New category')}}</a>
                <div class="btn-group ms-2 ms-lg-3">
                    <button type="button" class="btn btn-sm btn-outline-gray-600">{{__('Share')}}</button>
        ...
        <div class="card card-body shadow-sm table-wrapper table-responsive">
            ...
            <div class="alert alert-danger alert-dismissible fade show" role="alert">
                <span class="fas fa-bullhorn me-1"></span>
                {{__('You cannot delete this category since it has items asigned to it.')}}
                <button type="button" class="btn-close btn-sm" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
            @endif
            @if(session('success'))
            ...
            <x-table>
                ...
                <x-slot name="body">
                    ...
                    <x-table.row>
                        ...
                        <x-table.cell>{{ $category->date_for_humans }}</x-table.cell>
                        @can('manage-items', App\Models\User::class)
                        <x-table.cell>
                            ...
                            <x-button.link>
                                ...
                                <a class="dropdown-item rounded-top"
                                    href="{{ route('edit-category', ['id' => $category->id]) }}"><span
                                        class="fas fa-user-shield me-2"></span>{{__('Edit category')}}</a>
                                @endcan
                                @can('delete', $category)
                                <a onclick="confirm('Are you sure you want to remove this category?') || event.stopImmediatePropagation()"
                                    ...
                                    class="dropdown-item text-danger rounded-bottom"><span
                                        class="fas fa-user-times me-2"></span>{{__('Delete category')}}</a>
                                @endcan
                            </x-button.link>
                            @endif
                        </x-table.cell>
